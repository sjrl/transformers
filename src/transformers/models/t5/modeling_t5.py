...
class T5ForQuestionAnswering(T5PreTrainedModel):
    ...
    def parallelize(self, device_map=None):
        warnings.warn(
            "`T5ForQuestionAnswering.parallelize` is deprecated and will be removed in v5 of Transformers, you"
            " should load your model with `device_map='balanced'` in the call to `from_pretrained`. You can also"
            " provide your own `device_map` but it needs to be a dictionary module_name to device, so for instance"
            " {'encoder.block.0': 0, 'encoder.block.1': 1, ...}",
            FutureWarning,
        ...
        self.encoder.parallelize(self.device_map)
        self.decoder.parallelize(self.device_map)
        self.qa_outputs = self.qa_outputs.to(self.decoder.first_device)
        self.model_parallel = True
    ...
    def forward(
        ...
        if self.model_parallel:
            ...
            sequence_output = sequence_output.to(self.qa_outputs.weight.device)
