...
#mkdir -p ./experiments_lora/"$1"/"$2"
#  --output_dir experiments_lora/"$1"/"$2"/model/ \
mkdir -p ./experiments_lora/"${1}-encoder"/"$2"
nohup python run_qa.py \
  --model_name_or_path google/"$1" \
  --dataset_name squad_v2 \
  --output_dir experiments_lora/"${1}-encoder"/"$2"/model/ \
  ...
  --gradient_checkpointing True > experiments_lora/"${1}-encoder"/"$2"/run.log 2>&1 &
  #--gradient_checkpointing True > experiments_lora/"$1"/"$2"/run.log 2>&1 &
  #--torch_dtype float16 \
  #--tf32 True \
  #--bf16 \
  #--fp16 \
  #--optim adamw_bnb_8bit \  # Not needed for now w/ Lora
...
# Notes:
# can add tf32 w/ bf16 or fp16 for better throughput.
# load_in_8bit combined w/ tf32 and bf16 will take about 100 hours for 6 epochs. deberta-v3-large with fp16 took around 47 hours for 6 epochs.
